<template>
    <div class="navbar-item" v-if="item" :loading="item.loading || null" v-show="hasSearch">
        <div class="item-content" :active="isActive || null" :inactive="parentObj || null">
            <div class="container" @click="()=>{if(!parentObj)emit('callback', 'go', item.id, item.group)}">
            ...
            <div class="controls" v-if="parentObj">
                <div class="cas-controls">
                    <div class="ico-wr" @click="Mining.removeLayersFromObject([item.id], parentObj)"><ICross class="ico"/></div>
                </div>
            </div>
        </div>
